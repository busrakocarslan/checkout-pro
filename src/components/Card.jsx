...
function Show() {
  ...
  useEffect(()=>{// productCount değiştikçe yani ürün adedi değiştikçe toplamtotalPrice'in değişeceğini ayarlıyoruz.
    toplamTotalPrice();
  },[productCount])
  ...
  return (
    <div>
      ...
      {data.map(({ id, name, image, price }) => (
        <Product key={id} id={id} name={name} image={image} price={price}  handleCountChange={handleCountChange}   />
      ))}
      </div>
